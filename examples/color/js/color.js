;(function(undefined) {
	...
	var ColorModel = function(emitter) {
		...
		emitter.addListener('all', changeColor, 1);
		emitter.addListener('others', changeColor, 1);
	...
	var ContainerMediator = function(target, mediators) {
		...
		mediators.create(Array.from($('.widget')), WidgetMediator);
	};
	...
	var WidgetMediator = function(target, emitter, colorModel) {
		...
		function othersHandler(paramId) {
			if (id !== paramId) {
				$(target).css('background-color', colorModel.getColor());
			}
		}
