(function(snake) {
	...
	var Path = function(grid, emitter, time) {
		...
		emitter.addListener('keydown', function(params) {
			this.keys.push(params);
			if (this.keys.length > 2) {
				this.keys.length = 2;
			}
		}.bind(this));

		emitter.addListener('eating', this.addCell.bind(this));
	...
	Path.prototype.getNextPosition = function(direction) {
		...
		if (this.isCellBusy(nextCol, nextRow)) {
			if (!this.isOppositeDirection(direction)) {
				this.emitter.dispatch('end', ['eating-tail']);
			}
			return null;
		}
		if (!this.grid.isInBoundaries(nextCol, nextRow)) {
			this.emitter.dispatch('end', ['reached-boundaries']);
			return null;
		}
